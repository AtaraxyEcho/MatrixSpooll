...
class TestProjectManagerMore:
    ...
    @pytest.mark.unit
    def test_create_free_project_metadata_has_no_fixed_generation_route(self, tmp_path):
        pm = ProjectManager(tmp_path / "projects")
        ...
        assert project["content_mode"] == "free"
        assert project["generation_mode"] is None
        assert project["grid_storyboard"] is False
        assert project["episodes"] == []
        assert project["aspect_ratio"] == "21:9"
        assert (project_dir / "creations").is_dir()

    @pytest.mark.unit
    def test_create_free_project_metadata_rejects_fixed_workflow_options(self, tmp_path):
        pm = ProjectManager(tmp_path / "projects")
        pm.create_project("free-demo", content_mode="free")
        ...
        with pytest.raises(ValueError, match="grid_storyboard"):
            pm.create_project_metadata(
                "free-demo",
                "Free Demo",
                "Anime",
                "free",
                extras={"generation_mode": None, "grid_storyboard": True},
            )
