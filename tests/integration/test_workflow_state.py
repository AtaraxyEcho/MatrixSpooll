...
def _make_project(
    ...
    mode: str,
    *,
    generation_mode: str | None = "storyboard",
...
def test_free_project_bypasses_fixed_workflow(tmp_path: Path) -> None:
    pm, _project_path = _make_project(tmp_path, "free", generation_mode=None)

    status = WorkflowStateService(pm).get_status("demo")

    assert status.state == "CREATIVE"
    assert status.project.content_mode == "free"
    assert status.project.generation_mode is None
    assert status.project.workflow_applicable is False
    assert status.target is None
    assert status.blockers == []


def test_free_project_rejects_episode_workflow_status(tmp_path: Path) -> None:
    from lib.workflow_state import WorkflowRequestError

    pm, _project_path = _make_project(tmp_path, "free", generation_mode=None)

    with pytest.raises(WorkflowRequestError, match="do not have episodes"):
        WorkflowStateService(pm).get_status("demo", episode=1)
