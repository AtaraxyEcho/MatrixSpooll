...
def _make_profile(tmp_path: Path) -> Path:
    ...
    (profile / "CLAUDE.free.md").write_text("free top")
    # SKILL.md 变体配对
    ...
    (profile / ".claude" / "skills" / "video-workflow" / "SKILL.free.md").write_text("free skill")
    return profile
...
def test_resolve_for_free_picks_free_variants(tmp_path: Path) -> None:
    from lib.profile_manifest import resolve_profile_files_for_mode

    profile = _make_profile(tmp_path)
    mapping = resolve_profile_files_for_mode(profile, "free")

    assert mapping["CLAUDE.md"] == "CLAUDE.free.md"
    assert mapping[".claude/skills/video-workflow/SKILL.md"] == ".claude/skills/video-workflow/SKILL.free.md"
...
@pytest.mark.parametrize("mode", ["narration", "drama", "ad", "free"])
