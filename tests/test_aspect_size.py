...
@pytest.mark.parametrize("value", ALL_ASPECTS + [" 16：9 ", "18:32"])
def test_is_valid_aspect_ratio_accepts_positive_integer_ratios(value):
    assert is_valid_aspect_ratio(value)


@pytest.mark.parametrize("value", [None, "", "abc", "9:0", "0:16", "-9:16", "9", "9:16:1", "1.5:1"])
def test_is_valid_aspect_ratio_rejects_invalid_values(value):
    assert not is_valid_aspect_ratio(value)
