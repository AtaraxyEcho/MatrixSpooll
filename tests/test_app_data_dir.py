...
def app_data_dir_fn(monkeypatch: pytest.MonkeyPatch):
    monkeypatch.delenv("MATRIXSPOOLL_DATA_DIR", raising=False)
...
def test_absolute_matrixspooll_data_dir(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
):
    target = tmp_path / "custom-data"
    monkeypatch.setenv("MATRIXSPOOLL_DATA_DIR", str(target))
    monkeypatch.delenv("AI_ANIME_PROJECTS", raising=False)
...
def test_relative_matrixspooll_data_dir_resolved_against_project_root(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
):
    from lib.env_init import PROJECT_ROOT
    ...
    monkeypatch.setenv("MATRIXSPOOLL_DATA_DIR", rel_name)
    monkeypatch.delenv("AI_ANIME_PROJECTS", raising=False)
    app_data_dir_fn = _import_fresh()
...
def test_matrixspooll_data_dir_takes_precedence_over_legacy(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
):
    primary = tmp_path / "primary"
    secondary = tmp_path / "secondary"
    monkeypatch.setenv("MATRIXSPOOLL_DATA_DIR", str(primary))
...
def test_empty_env_value_falls_through_to_default(
    ...
    """`MATRIXSPOOLL_DATA_DIR=` (empty) should not be treated as a path."""
    from lib.env_init import PROJECT_ROOT

    monkeypatch.setenv("MATRIXSPOOLL_DATA_DIR", "")
...
def test_result_is_cached_within_single_call_sequence(
    ...
    target_a = tmp_path / "a"
    target_b = tmp_path / "b"
    monkeypatch.setenv("MATRIXSPOOLL_DATA_DIR", str(target_a))
    monkeypatch.delenv("AI_ANIME_PROJECTS", raising=False)
    app_data_dir_fn = _import_fresh()

    first = app_data_dir_fn()
    # mutate env, but don't reset cache — same result expected.
    monkeypatch.setenv("MATRIXSPOOLL_DATA_DIR", str(target_b))
