...
INTENTIONAL_TASKS_INDEX_DROPS = {
    ...
    "d2c4e6f8a0b2": frozenset({"idx_tasks_project_id"}),
...
# Identity and runtime-locator migrations after this revision intentionally
# reject downgrade. This test owns the older table-rebuild segment only.
REVERSIBLE_INDEX_TEST_HEAD = "b5e7f9a1c3d5"
...
def test_downgrade_keeps_indexes(alembic_cfg, revision):
    ...
    从可逆段顶端逐级降到被测迁移，而不是从 base 升上来：升级路径在 ``ea2e1a477bbf`` 处同样会丢掉
    ...
    command.upgrade(cfg, REVERSIBLE_INDEX_TEST_HEAD)
...
def test_full_chain_downgrade_only_drops_intentional_indexes(alembic_cfg):
    """可逆迁移段逐级降到 base，每步只丢该迁移显式删除的 tasks 索引。
    ...
    for revision in (s.revision for s in script.walk_revisions("base", REVERSIBLE_INDEX_TEST_HEAD)):
