...
class TestProjectsRouter:
    ...
    def test_create_requires_binary_generation_mode(self, tmp_path, monkeypatch):
        ...
        with client:
            ...
            # 自由创作是内容模式，不是第三条生成路线；该模式必须把路线持久化为 null。
            free = client.post(
                "/api/v1/projects",
                json={"name": "free-project", "title": "Free", "content_mode": "free"},
            )
            assert free.status_code == 200, free.text
            assert free.json()["project"]["generation_mode"] is None
            assert free.json()["project"]["grid_storyboard"] is False

            free_with_route = client.post(
                "/api/v1/projects",
                json={
                    "name": "free-with-route",
                    "title": "Free",
                    "content_mode": "free",
                    "generation_mode": "storyboard",
                },
            )
            assert free_with_route.status_code == 422

            free_with_grid = client.post(
                "/api/v1/projects",
                json={"name": "free-with-grid", "title": "Free", "content_mode": "free", "grid_storyboard": True},
            )
            assert free_with_grid.status_code == 422
