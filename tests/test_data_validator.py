...
class TestDataValidator:
    @pytest.mark.unit
    def test_validate_free_project_without_fixed_workflow_fields(self, tmp_path):
        project_dir = tmp_path / "projects" / "free-demo"
        _write_json(
        ...
        result = DataValidator(projects_root=str(tmp_path / "projects")).validate_project("free-demo")
    ...
    def test_validate_free_project_rejects_fixed_route_and_invalid_ratio(self, tmp_path):
        ...
        assert any("generation_mode" in error for error in result.errors)
        assert any("画面比例" in error for error in result.errors)
