...
def test_session_manager_wires_env_resolved_roots_into_policy(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    """SessionManager 负责 env 解析（MATRIXSPOOLL_LOG_DIR / MATRIXSPOOLL_PROFILE_DIR /
    ...
    monkeypatch.setenv("MATRIXSPOOLL_LOG_DIR", str(external_logs))
    monkeypatch.setenv("MATRIXSPOOLL_PROFILE_DIR", str(external_profile))
