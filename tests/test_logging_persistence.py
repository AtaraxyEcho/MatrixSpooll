...
def isolated_data_dir(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Iterator[Path]:
    ...
    data_root.mkdir()
    project_root.mkdir()
    monkeypatch.setenv("MATRIXSPOOLL_DATA_DIR", str(data_root))
    monkeypatch.delenv("MATRIXSPOOLL_LOG_DIR", raising=False)
...
def test_resolve_log_dir_relative_path_resolves_against_project_root(
    ...
    """相对路径 MATRIXSPOOLL_LOG_DIR 必须基于 PROJECT_ROOT 解析。"""
    project_root = tmp_path / "repo"
    project_root.mkdir()
    monkeypatch.setattr(logging_config, "PROJECT_ROOT", project_root)
    monkeypatch.setenv("MATRIXSPOOLL_LOG_DIR", "var/log/matrixspooll")

    assert logging_config.resolve_log_dir() == project_root / "var" / "log" / "matrixspooll"
...
def test_migrate_moves_legacy_dir_when_new_absent(isolated_data_dir: Path) -> None:
    ...
    old_dir.mkdir()
    (old_dir / "matrixspooll.log").write_text("old content\n", encoding="utf-8")
    (old_dir / "matrixspooll.log.2026-05-20").write_text("rotated\n", encoding="utf-8")
    ...
    assert (new_dir / "matrixspooll.log").read_text(encoding="utf-8") == "old content\n"
    assert (new_dir / "matrixspooll.log.2026-05-20").exists()
...
def test_migrate_skips_when_both_have_content(isolated_data_dir: Path) -> None:
    ...
    old_dir.mkdir()
    new_dir.mkdir()
    (old_dir / "matrixspooll.log").write_text("old\n", encoding="utf-8")
    (new_dir / "matrixspooll.log").write_text("new\n", encoding="utf-8")

    logging_config.migrate_legacy_log_dir()

    # 两边都原样保留，不静默覆盖
    assert (old_dir / "matrixspooll.log").read_text(encoding="utf-8") == "old\n"
    assert (new_dir / "matrixspooll.log").read_text(encoding="utf-8") == "new\n"
...
def test_migrate_proceeds_when_new_dir_empty(isolated_data_dir: Path) -> None:
    ...
    (old_dir / "matrixspooll.log").write_text("payload\n", encoding="utf-8")
    (old_dir / "matrixspooll.log.2026-05-20").write_text("rotated\n", encoding="utf-8")
    ...
    assert (new_dir / "matrixspooll.log").read_text(encoding="utf-8") == "payload\n"
    assert (new_dir / "matrixspooll.log.2026-05-20").read_text(encoding="utf-8") == "rotated\n"
...
def test_migrate_skips_when_log_dir_env_set(isolated_data_dir: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    old_dir = isolated_data_dir / "data" / "logs"
    old_dir.mkdir()
    (old_dir / "matrixspooll.log").write_text("keep me\n", encoding="utf-8")
    monkeypatch.setenv("MATRIXSPOOLL_LOG_DIR", str(isolated_data_dir / "custom"))

    logging_config.migrate_legacy_log_dir()

    # 用户显式设了 LOG_DIR，旧目录原地保留
    assert (old_dir / "matrixspooll.log").read_text(encoding="utf-8") == "keep me\n"


def test_migrate_noop_when_paths_equal(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    """MATRIXSPOOLL_DATA_DIR == PROJECT_ROOT 时旧新路径解析到同一处，不要把目录自己 rename 到自己。"""
    ...
    try:
        logs = tmp_path / "logs"
        logs.mkdir()
        (logs / "matrixspooll.log").write_text("hi\n", encoding="utf-8")

        logging_config.migrate_legacy_log_dir()  # 不抛

        assert (logs / "matrixspooll.log").read_text(encoding="utf-8") == "hi\n"
    finally:
        app_data_dir_mod._reset_for_tests()
...
def test_migrate_failure_logs_error(
    ...
    old_dir = isolated_data_dir / "data" / "logs"
    old_dir.mkdir()
    (old_dir / "matrixspooll.log").write_text("stuck\n", encoding="utf-8")
    ...
    # 旧 dir 仍在
    assert (old_dir / "matrixspooll.log").exists()
...
def test_setup_logging_file_false_skips_file_handler(isolated_log_dir: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    # 注意：isolated_log_dir 把 MATRIXSPOOLL_LOG_DIR 设到 tmp_path/logs 但还没创建
