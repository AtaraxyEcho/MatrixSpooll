...
from server.auth import CurrentUser, CurrentUserFlexible, database_auth_initialized, is_auth_enabled
...
async def resolve_project_access(
    ...
    *,
    ...
    try:
        registry = await session.scalar(select(ProjectRegistry).where(ProjectRegistry.name == normalized_name))
    except Exception:
        logger.exception("项目授权查询失败 project=%s", normalized_name)
        raise
    ...
    if registry is None and user.id == DEFAULT_USER_ID and (not is_auth_enabled() or legacy_uninitialized):
    ...
    if member is None or ROLE_ORDER.get(member.role, 0) < ROLE_ORDER.get(required_role, 0):
...
async def remove_project_registration(session: AsyncSession, project_name: str) -> None:
    """Remove registry rows after a project directory is deleted."""
...
async def list_accessible_projects(user_id: str, session: AsyncSession) -> list[ProjectRegistry]:
    """Return only projects visible to one user, ordered by update time."""

    stmt = (
        select(ProjectRegistry)
        .join(ProjectMember, ProjectMember.project_id == ProjectRegistry.id)
        .where(ProjectMember.user_id == user_id)
        .order_by(ProjectRegistry.updated_at.desc(), ProjectRegistry.name.asc())
    )
