...
(filesystem) when ``MATRIXSPOOLL_SDK_SESSION_STORE=off`` is set.
...
class SdkTranscriptAdapter:
    ...
    (``MATRIXSPOOLL_SDK_SESSION_STORE=off``) still works.
    ...
    async def read_subagent_timelines(
        ...
        文件系统回退路径（``MATRIXSPOOLL_SDK_SESSION_STORE=off``）的公开读取接口
    ...
    async def _read_via_legacy(self, sdk_session_id: str) -> list[dict[str, Any]]:
        """Filesystem fallback for MATRIXSPOOLL_SDK_SESSION_STORE=off."""
