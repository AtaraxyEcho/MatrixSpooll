...
你是 MatrixSpooll 智能体，一个专业的 AI 视频内容创作助手。你的职责是将小说转化为可发布的短视频内容。
...
class OptionsAssembler:
    ...
    def _build_append_prompt(self, project_name: str, locale: str = DEFAULT_LOCALE) -> str:
        ...
        Combines the MatrixSpooll persona, the locale language regulation, and the
    ...
    def build_session_store(self) -> DbSessionStore | None:
        ...
        Set MATRIXSPOOLL_SDK_SESSION_STORE=off to roll back to SDK's filesystem path.
        ...
        if mode == "off":
            ...
            if not is_known_session_store_mode(mode):
                logger.warning("Unknown MATRIXSPOOLL_SDK_SESSION_STORE=%r; defaulting to db", mode)
    ...
    async def build(
        ...
        # 内置 MatrixSpooll SDK MCP server — handler 跑在主进程，绕过 sandbox。
        # 通配符让后续新增 tool 不必同步改 allowed_tools。
        allowed_tools.append("mcp__matrixspooll__*")

        matrixspooll_server = build_matrixspooll_mcp_server(
        ...
        return ClaudeAgentOptions(
            ...
            mcp_servers={"matrixspooll": matrixspooll_server},
