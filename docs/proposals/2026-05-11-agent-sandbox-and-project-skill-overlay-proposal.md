...
提升 MatrixSpooll 智能体的自由度,同时不扩大 secrets 外泄面。具体两件事:
...
SDK 0.1.80 的 `SandboxSettings` 只管命令执行行为,不管文件和网络。文件/网络限制走 `permissions.deny`,扩展到当前漏掉的敏感文件(`projects/.matrixspooll.db`、`projects/.system_config.json.bak`、`agent_runtime_profile/.claude/settings.json` 等)。
...
- **agent 不能读取**`.env` / `projects/.matrixspooll.db` / `projects/.system_config.json.bak` / `vertex_keys/**` / `agent_runtime_profile/.claude/settings.json` 等敏感文件
- **agent 不能写项目目录外**
...
- 写前 Checkpoint / rewind(SDK 0.1.80 的 `enable_file_checkpointing` 与 MatrixSpooll 0.13.0 起用的 DB session store 互斥,需要单独评估技术路径后立项)
...
3. **沙箱不兼容的 Bash 命令**(`docker`、`watchman` 等):MatrixSpooll 用不到,影响小。如未来引入需走 `excludedCommands`。
