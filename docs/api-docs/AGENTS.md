...
本目录只保存导航：官方来源链接、MatrixSpooll 依赖的主题和稳定代码入口。运行时事实仍归 registry、backend 与 endpoint 声明，不从索引反向加载。
...
3. 更新本 id 的官方入口、MatrixSpooll 实际依赖页面和代码 `path::symbol`。共享文档可重复链接，不复制页面内容或数值。
