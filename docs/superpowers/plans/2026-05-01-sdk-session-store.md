...
logger = logging.getLogger("matrixspooll.session_store.import")
...
"""SessionManager._build_session_store reads MATRIXSPOOLL_SDK_SESSION_STORE."""
...
async def test_store_enabled_by_default(monkeypatch, session_factory, tmp_path):
    monkeypatch.delenv("MATRIXSPOOLL_SDK_SESSION_STORE", raising=False)
...
async def test_store_off_returns_none(monkeypatch, session_factory, tmp_path):
    monkeypatch.setenv("MATRIXSPOOLL_SDK_SESSION_STORE", "off")
    sm = SessionManager(data_dir=tmp_path, projects_root=tmp_path)
...
def _build_session_store(self):
    ...
    mode = os.getenv("MATRIXSPOOLL_SDK_SESSION_STORE", "db")
    if mode == "off":
        return None
...
class SdkTranscriptAdapter:
    ...
    def read_raw_messages(self, sdk_session_id: str | None) -> list[dict[str, Any]]:
        ...
        if self._store is None:
            # Roll-back path: MATRIXSPOOLL_SDK_SESSION_STORE=off — fall through to
    ...
    # --- legacy fallback (MATRIXSPOOLL_SDK_SESSION_STORE=off) -------------------
...
try:
    ...
    await migrate_local_transcripts_to_store(
        ...
        data_dir=PROJECT_ROOT / "projects" / ".matrixspooll_data",
    )
except Exception:
    logger.exception("session-store transcript migration failed (non-fatal)")
```

> data_dir 选址需与 MatrixSpooll 当前 `.matrixspooll.db` 同级，确保 docker volume 自然带上；
...
steps:
  - name: Set DATABASE_URL (postgres)
    ...
    run: echo "DATABASE_URL=postgresql+asyncpg://test:test@localhost:5432/matrixspooll_test" >> $GITHUB_ENV
...
```bash
docker run --rm -d --name matrixspooll-pg-test -e POSTGRES_USER=test -e POSTGRES_PASSWORD=test \
  -e POSTGRES_DB=matrixspooll_test -p 5432:5432 postgres:16

DATABASE_URL=postgresql+asyncpg://test:test@localhost:5432/matrixspooll_test \
  uv run alembic upgrade head

DATABASE_URL=postgresql+asyncpg://test:test@localhost:5432/matrixspooll_test \
  uv run python -m pytest tests/agent_session_store/ -v

docker stop matrixspooll-pg-test
```
...
| §4.3 环境变量回滚 MATRIXSPOOLL_SDK_SESSION_STORE | Task 10 |
...
单 worker 部署足够。**多 worker 部署的额外锁留给 Task 17 增补**——但 MatrixSpooll 当前
