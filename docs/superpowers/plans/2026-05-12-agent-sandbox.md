...
- `Bash("cat /app/projects/.matrixspooll.db")` → 同上
...
AUTH_ALLOWED_PREFIXES: tuple[str, ...] = ("AUTH_", "ASSISTANT_", "MATRIXSPOOLL_")
...
注意：这条策略 **激进**（删除所有非白名单 env）。如果项目有其他业务 env（如 `OPENAI_API_KEY` 给某个非 MatrixSpooll 代码用），会被误删。如不放心，可以改为只删 `OTHER_PROVIDER_ENV_KEYS + ANTHROPIC_ENV_KEYS` 的精确名单：
...
```json
{
  "permissions": {
    "deny": [
      ...
      "Read(//app/projects/.matrixspooll.db)",
      "Read(//app/projects/.matrixspooll.db-*)",
      ...
      "Edit(//app/projects/.matrixspooll.db)",
      "Edit(//app/projects/.matrixspooll.db-*)",
...
```bash
docker build -t matrixspooll:sandbox-test .
docker run --rm matrixspooll:sandbox-test which bwrap
```
...
MatrixSpooll 启动会进行严格的安全检查 — sandbox 工具缺失即拒绝启动。
...
- [ ] `.matrixspooll.db` 当前可读，未纳入 denyRead
  ...
  - 解除条件：issue #519 完成 skill 脚本→SDK 原生 tool 重构后，将 `.matrixspooll.db` 加回 denyRead 并反向断言
...
- [ ] agent 可访问白名单内的域名
  ...
  - 扩展：`MATRIXSPOOLL_SANDBOX_EXTRA_ALLOWED_DOMAINS=custom.io,*.internal.corp uv run uvicorn ...` 逗号分隔追加
