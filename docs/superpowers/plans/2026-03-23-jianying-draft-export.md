...
**Goal:** 将 MatrixSpooll 单集已生成的视频片段导出为剪映草稿 ZIP，用户解压到本地剪映草稿目录后直接在剪映中打开编辑。
...
将 MatrixSpooll 单集已生成的视频片段导出为剪映草稿 ZIP。
...
class TestReplacePaths:
    ...
    def test_replaces_tmp_prefix_in_json(self, tmp_path):
        ...
        data = {
            "materials": {
                "videos": [
                    {"path": "/tmp/matrixspooll_jy_abc/草稿/assets/s1.mp4"},
                    {"path": "/tmp/matrixspooll_jy_abc/草稿/assets/s2.mp4"},
                ]
            },
            "other": "no change",
        }
        ...
        svc._replace_paths_in_draft(
            json_path=json_path,
            tmp_prefix="/tmp/matrixspooll_jy_abc/草稿/assets",
...
def export_episode_draft(
    ...
    tmp_dir = Path(tempfile.mkdtemp(prefix="matrixspooll_jy_"))
...
- **草稿目录输入**：`<input type="text">`，`onChange` 时同步写 `localStorage.setItem("matrixspooll_jianying_draft_path", value)`
