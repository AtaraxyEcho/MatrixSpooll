...
**Goal:** 在 MatrixSpooll 上传入口增加 `.txt / .md / .docx / .epub / .pdf` 多格式支持并一次性规范化为 UTF-8 文本，下游消费者零改动；新增前端冲突协商弹窗、首次上传自动触发分析、原始格式下载入口；启动时幂等迁移历史项目编码。
...
def test_startup_migration_creates_marker_after_run(tmp_path: Path):
    ...
    marker = project / ".matrixspooll" / "source_encoding_migrated"
...
def test_startup_migration_skips_already_marked(tmp_path: Path):
    ...
    bad.write_bytes(("第一章\n" * 30).encode("gbk"))
    marker_dir = project / ".matrixspooll"
    marker_dir.mkdir()
...
def test_startup_migration_isolates_project_failures(tmp_path: Path, monkeypatch):
    ...
    summary = asyncio.run(_migrate_source_encoding_on_startup(tmp_path))
    assert (good / ".matrixspooll" / "source_encoding_migrated").exists()
    assert (bad / ".matrixspooll" / "source_encoding_migrated").exists()
    assert (bad / ".matrixspooll" / "migration_errors.log").exists()
```
...
- 首次上传自动触发分析
- 启动时一次性迁移历史项目源文件编码（幂等，失败不阻塞启动；明细见 projects/<name>/.matrixspooll/migration_errors.log）
```
