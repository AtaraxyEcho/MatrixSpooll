...
```bash
cd /Users/pollochen/MyProjects/MatrixSpooll/.worktrees/fix/a11y-focus-visible-219
...
```bash
cd /Users/pollochen/MyProjects/MatrixSpooll/.worktrees/fix/a11y-focus-visible-219/frontend
node -e '
...
Run: `gh run watch --repo MockMine/MatrixSpooll` （或 `gh pr checks <pr-number>` 若已创建 PR）
