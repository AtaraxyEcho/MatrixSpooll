...
**Goal:** 在不动 schema 前提下，把分集节奏铁则、动态视觉规则、资产防崩与资产布局四类 prompt 增强通过新建 `lib/prompt_rules/` 模块分层注入到 `prompt_builders_script.py`、`generate_asset.py` 与 step1 subagent，让 MatrixSpooll script 阶段输出对齐小云雀 V2.1 / Seedance 爆款 prompt 水平。

**Architecture:** 新建 `lib/prompt_rules/` 作为规则单一真相源；Python 端通过 `MATRIXSPOOLL_PROMPT_RULES_V2` 灰度开关控制注入；subagent .md 静态贴文本，靠 substring 锚点测试防漂移；不改下游 generate-storyboard / generate-video，下游零改动自动受益。
...
def test_default_is_on(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.delenv("MATRIXSPOOLL_PROMPT_RULES_V2", raising=False)
    assert is_v2_enabled() is True


def test_explicit_off(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("MATRIXSPOOLL_PROMPT_RULES_V2", "off")
...
def test_other_value_treated_as_on(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("MATRIXSPOOLL_PROMPT_RULES_V2", "true")
    assert is_v2_enabled() is True
```
...
所有 Python 端注入受 `MATRIXSPOOLL_PROMPT_RULES_V2` 环境变量控制（默认 on）。
"""
...
def is_v2_enabled() -> bool:
    return os.environ.get("MATRIXSPOOLL_PROMPT_RULES_V2", "on").lower() != "off"
...
git commit -m "feat(prompt_rules): bootstrap 包 + MATRIXSPOOLL_PROMPT_RULES_V2 灰度开关"
```
...
def test_drama_v2_on_injects_all(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("MATRIXSPOOLL_PROMPT_RULES_V2", "on")
...
def _wrap_prompt(asset_type: str, description: str) -> tuple[str, str | None]:
    ...
    MATRIXSPOOLL_PROMPT_RULES_V2=off 时退回原始 description（与 negative_prompt=None）。
    """
...
```bash
cd projects/$PROJ
MATRIXSPOOLL_PROMPT_RULES_V2=off uv run python ../../agent_runtime_profile/.claude/skills/generate-script/scripts/generate_script.py --episode 1 --dry-run > /tmp/prompt_old.txt
cd ../..
```
...
```bash
cd projects/$PROJ
MATRIXSPOOLL_PROMPT_RULES_V2=on uv run python ../../agent_runtime_profile/.claude/skills/generate-script/scripts/generate_script.py --episode 1 --dry-run > /tmp/prompt_new.txt
cd ../..
```
...
```bash
cd projects/$PROJ
MATRIXSPOOLL_PROMPT_RULES_V2=on uv run python ../../agent_runtime_profile/.claude/skills/generate-script/scripts/generate_script.py --episode 1
```
...
```bash
cd projects/$PROJ
MATRIXSPOOLL_PROMPT_RULES_V2=on uv run python ../../agent_runtime_profile/.claude/skills/generate-assets/scripts/generate_asset.py --type character --name <一个待生成角色>
```
