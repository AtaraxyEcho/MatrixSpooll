...
应用代码 logger.info(...)
   ...
   └─ TimedRotatingFileHandler → PROJECT_ROOT/logs/matrixspooll.log
            │
            └─ 每日 midnight 切到 matrixspooll.log.YYYY-MM-DD，最多 7 份

用户在 Settings 点「下载诊断日志」
   │
   GET /api/v1/system/logs/download
   │
   └─ ZipFile 流式打包 logs/matrixspooll.log* + diagnostics.txt
      → 返回 application/zip
```
...
- 日志目录由新增函数 `resolve_log_dir()` 决定：
  1. `MATRIXSPOOLL_LOG_DIR` env（绝对路径或相对 `PROJECT_ROOT`）
...
- 逃生口：`MATRIXSPOOLL_LOG_FILE_DISABLED` 取值 `1` / `true` / `yes`（大小写不敏感）时跳过 file handler 注册
...
2. 遍历 `resolve_log_dir()` 下所有 `matrixspooll.log*` 文件
...
4. zip 写完后 `seek(0)`，包装成 `StreamingResponse(spooled_file, media_type="application/zip", headers={"Content-Disposition": f'attachment; filename="matrixspooll-diagnostics-{ts}.zip"'})`；StreamingResponse 消费完后 SpooledTemporaryFile 自动关闭并删除磁盘 backing file

文件名格式：`matrixspooll-diagnostics-YYYY-MM-DD-HHMM.zip`（本地时区即可）。
...
| `test_file_handler_disabled_by_env` | `MATRIXSPOOLL_LOG_FILE_DISABLED=1` 时不注册 |
...
| `test_custom_log_dir` | `MATRIXSPOOLL_LOG_DIR=tmp_path/foo` 生效 |
...
| 老用户磁盘空间不足 | 提供 `MATRIXSPOOLL_LOG_FILE_DISABLED=1` 逃生口 |
