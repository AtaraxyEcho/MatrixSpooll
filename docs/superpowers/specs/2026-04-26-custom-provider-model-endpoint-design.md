...
MatrixSpooll 的自定义供应商当前以 `CustomProvider.api_format ∈ {openai, google, newapi}` 在供应商层决定调用协议。这隐含了「一个供应商 = 一种协议」的假设。
...
- 仅内置前端调用，按 MatrixSpooll 风格不留 backwards-compat shim
