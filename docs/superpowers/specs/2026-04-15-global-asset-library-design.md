...
MatrixSpooll 当前项目之间互相隔离，同一用户反复创作相似世界观时（如古风短剧系列），角色 / 线索需要重复录入，无法跨项目复用。本设计引入**全局资产库**（可跨项目复用的人物 / 场景 / 道具仓库），并借此机会**彻底重构旧 `clue` 概念**：线索拆分为独立的 `scene`（场景）和 `prop`（道具）两类一级对象，删除 `importance` 字段。
...
2. 扫 `projects/` 每个项目目录（跳过 `_global_assets/`、`.matrixspooll.db` 等）
