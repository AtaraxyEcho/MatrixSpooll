...
| 启用方式 | 默认开启，环境变量 `MATRIXSPOOLL_SDK_SESSION_STORE=off` 可回滚 |
...
**多用户隔离不靠 project_key**：
- MatrixSpooll 当前每个项目独立 cwd（`projects/<project_name>/`），项目间天然隔离
...
新增环境变量：`MATRIXSPOOLL_SDK_SESSION_STORE`（默认 `"db"`，可设 `"off"`）
...
| 阶段 | 默认值 | 验证 |
|---|---|---|
| 首发 | `MATRIXSPOOLL_SDK_SESSION_STORE=db` 默认开 | 全量回归测试 + 1–2 周生产观察 |
...
1. **零路径硬编码** — `~/.claude/projects/` 还是 `CLAUDE_CONFIG_DIR/projects/` 还是
   docker 挂载点完全由 SDK 决定，迁移代码只传 MatrixSpooll 的项目 cwd 给 SDK。
...
`lib/agent_session_store/store.py` 用 `logger = logging.getLogger("matrixspooll.session_store")`：
...
| 7 | 设 `MATRIXSPOOLL_SDK_SESSION_STORE=off` 重启 → 退化到旧 jsonl 路径仍可用 |
