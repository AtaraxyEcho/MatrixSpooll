...
MatrixSpooll 现有 prompt 体系（`lib/prompt_builders_script.py` 的 `build_drama_prompt` / `build_narration_prompt`、step1 拆分 subagent、`generate_asset.py` 资产生图）相比小云雀 V2.1 与 Seedance Excel 的爆款 prompt 模板存在以下系统性缺口：

| 维度 | 小云雀 / Seedance | MatrixSpooll 现状 | gap |
|---|---|---|---|
...
**目标**：在**不动 schema**的前提下，把上述能力补齐，让 MatrixSpooll 输出的 `image_prompt.scene` / `video_prompt.action` 文本和资产生图 prompt 直接对齐爆款博主水平。
...
> 现有约束「每个片段仅选择一种镜头运动」（`camera_motion` 字段说明）保留不动——这是 MatrixSpooll 为视频生成稳定性的主动保守，与"动态优先"指向画面 / 动作内容不冲突。
...
### 6.1 灰度开关
`MATRIXSPOOLL_PROMPT_RULES_V2=off` 重启 server → Python 端全部退回旧文本。subagent .md 回滚靠 git revert。
...
### 6.6 一键回滚预案
1. `MATRIXSPOOLL_PROMPT_RULES_V2=off` 重启
...
- `MATRIXSPOOLL_PROMPT_RULES_V2=off` → False，大小写不敏感
...
```bash
MATRIXSPOOLL_PROMPT_RULES_V2=off uv run python .../generate_script.py --episode 1 --dry-run > /tmp/prompt_old.txt
MATRIXSPOOLL_PROMPT_RULES_V2=on  uv run python .../generate_script.py --episode 1 --dry-run > /tmp/prompt_new.txt
