...
### 0.2 红线 2 范围调整：`.matrixspooll.db` 暂从 `denyRead` 清单移除 ✅ 已通过 issue #519 解除

> **2026-05 更新**：入队链路（5 个 enqueue skill 脚本）已改造为 SDK in-process MCP tool（`server/agent_runtime/sdk_tools/`），handler 跑在 server 主进程，不经 sandbox。`projects/.matrixspooll.db` + WAL 辅助文件已加回 `_SENSITIVE_RELATIVE_FILES` / `_SENSITIVE_RELATIVE_GLOBS`。原文保留作决策线索。

skill 脚本通过 `lib.generation_queue_client` 直连本地 SQLite (`projects/.matrixspooll.db`) 入队任务——sandbox 把 db 加进 `denyRead` 会让任务队列功能整体失效。当前**接受 db 可被 sandbox 内进程读**（含明文 `agent_credential` / `credential` / `custom_provider` 的 `api_key` 字段）的限制。
...
> **2026-05 更新**：所有 provider HTTP 调用已迁到 in-process MCP tool（主进程跑，不经 sandbox），sandbox 内不再需要放行 provider 域名。`_DEFAULT_SANDBOX_ALLOWED_DOMAINS` 仅保留 Anthropic SDK 自身 + 通用 dev 域名（docs / 包仓库等）。`MATRIXSPOOLL_SANDBOX_EXTRA_ALLOWED_DOMAINS` ENV hook 已删除。原文保留作决策线索。

`sandbox.network.allowedDomains` 默认覆盖：
- MatrixSpooll 内置 provider 域名：`anthropic.com` / `*.anthropic.com` / `*.volces.com` / `x.ai` / `*.x.ai` / `*.googleapis.com` / `*.google.com` / `vidu.cn` / `*.vidu.cn` / `*.viduai.cn`
...
`MATRIXSPOOLL_SANDBOX_EXTRA_ALLOWED_DOMAINS`（逗号分隔）允许运维补充自定义 provider 域名。**spec 第 § 2 决策 5 文字相应修订**。
...
`_is_path_allowed` 规则 1 原判断"路径在 `projects/` 下且不在 cwd 内 → 跨项目拒"，把 `projects/` 根下直放的文件（如 `.matrixspooll.db`）误判为跨项目读。修正为"路径落入 `projects/<某子目录>/` 才算跨项目"，根下直放文件放行。
...
2. agent 不能读取 `.env` / `projects/.system_config.json{,.bak}` / `vertex_keys/**` / `agent_runtime_profile/.claude/settings.json` — `projects/.matrixspooll.db*` **暂不在范围**，见 §0.2
3. agent 不能写项目目录外
...
- 决策 5：~~沙箱网络默认放行~~ → **维护内置 provider + 常用 dev 域名白名单，ENV `MATRIXSPOOLL_SANDBOX_EXTRA_ALLOWED_DOMAINS` 扩展**（SDK 0.1.80 不支持 `["*"]` 全放行，见 §0.3）
...
| 新增 `_build_sandbox_allowed_domains()` | 默认清单 + `MATRIXSPOOLL_SANDBOX_EXTRA_ALLOWED_DOMAINS` ENV 扩展 | §0.3 |
...
为什么不放 deny rules：
- settings.json 里的 deny 路径是静态字符串，无法用 `self.project_root` 动态拼，宿主开发态路径不固定（`/Users/<user>/MatrixSpooll`、worktree 子路径等）—— 静态写无法命中
...
Required for MatrixSpooll agent runtime. Install bubblewrap:
...
- 不提供 `MATRIXSPOOLL_DISABLE_SANDBOX` 或类似环境开关
...
| 2 | macOS + Linux + Docker | sandbox enabled，agent Bash 跑 `cat /app/.env` / `cat /app/projects/.matrixspooll.db` | 拒绝（permission denied / sandbox violation） | 阳性 → 加 `Read(...)` deny rule；阴性（拒绝失败）→ 重审决策 1 可行性 |
...
> 已知限制：`projects/.matrixspooll.db` 暂时未纳入 denyRead（skill 入队脚本需要直读 SQLite），由 issue #519 跟踪。该 db 内的 provider 密钥目前依赖应用层"不在 Bash 中导出"防线，而非内核级阻断。
>
...
| Sandbox 不兼容的 Bash 命令（`docker` / `watchman` 等） | SDK 文档 | MatrixSpooll 不用；未来引入需走 `excludedCommands` |
...
| `.matrixspooll.db` 含明文 provider 密钥仍可被 sandbox 内 skill 脚本读取 | §0.2 实测限制 | 已开 follow-up issue：把 generation queue 入队链路改造为 SDK 原生 tool（in-process MCP），让 agent 不再 Bash → python 脚本路径访问 db。改造完成后 db 加回 `denyRead`。当前缓解：`agent_credential` / `credential` / `custom_provider` 表的 `api_key` 字段保持 application-level 加密的优先级提升 |
| 用户自定义 provider 域名未在默认白名单内 → skill HTTP 请求被沙箱拦 | §0.3 SDK 限制 | `MATRIXSPOOLL_SANDBOX_EXTRA_ALLOWED_DOMAINS` ENV 扩展。后续 PR 加"新域请求 → 用户确认 → 永久放行"流，避免运维手动维护 |
