...
**日期**：2026-03-04
**Issue**：[#48](https://github.com/MockMine/MatrixSpooll/issues/48)
**状态**：已批准
...
- **DATABASE_URL 解析**：从环境变量 `DATABASE_URL` 读取
  - 默认值：`sqlite+aiosqlite:///{app_data_dir()/.matrixspooll.db}`（`app_data_dir()` 默认 `PROJECT_ROOT/projects`，可经 `MATRIXSPOOLL_DATA_DIR` 覆盖；开发态即 `projects/.matrixspooll.db`）
  - PostgreSQL：`postgresql+asyncpg://user:pass@host:5432/matrixspooll`
...
```bash
# 数据库配置（默认使用 SQLite）
# SQLite（开发/单机）: sqlite+aiosqlite:///./projects/.matrixspooll.db
# PostgreSQL（生产）:  postgresql+asyncpg://user:pass@host:5432/matrixspooll
# DATABASE_URL=sqlite+aiosqlite:///./projects/.matrixspooll.db
```
