...
MatrixSpooll 当前 `ALLOWED_EXTENSIONS["source"]` 声明支持 `.txt / .md / .doc / .docx`，但下游所有消费者（`ProjectManager._read_source_files`、agent skill `normalize_drama_script.py` / `split_episode.py` / `peek_split_point.py`、`analyze-characters-clues` agent）均通过硬编码 `encoding="utf-8"` 读取，且 `_read_source_files` 用 `try/except Exception` 静默跳过失败文件。三类直接故障：
...
async def _migrate_source_encoding_on_startup():
    ...
    for project_dir in projects_root.iterdir():
        if not project_dir.is_dir():
            continue
        marker = project_dir / ".matrixspooll" / "source_encoding_migrated"
        if marker.exists():
            continue
        try:
            ...
            # 失败明细写 project_dir / ".matrixspooll" / "migration_errors.log"
```
