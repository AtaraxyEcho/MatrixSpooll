...
> 用途：驱动 MatrixSpooll（小说→短视频，i2v 图生视频流水线）剧本生成写作指导（LLM 产出 `image_prompt` / `video_prompt`）的迭代。
> 对应议题：https://github.com/MockMine/MatrixSpooll/issues/1092（设计对齐决议见该 issue 评论区）。
> 范围：8 家 MatrixSpooll 实际接入的供应商，仅采信**官方**文档站/官方博客/模型卡/官方 Help Center。查不到官方指南的，如实记为"未找到官方指南"，不以第三方教程或模型记忆顶替。
> 调研日期：2026-07-13
...
> **对 MatrixSpooll 的启示（仅陈述共性事实，不作改稿）**：官方普遍支持"要素分层 + 具体可视化 + 叙述句"，且普遍反对空泛质量词堆砌。"过于精简"（如 scene 缺光线/氛围层）与主流官方口径相悖；补齐主体/环境/光线/镜头语言/氛围层次有跨厂商官方依据。长度上官方几乎不设硬指标，倾向"该具体的地方具体"。

---

### Q2 i2v 图生视频 prompt 总体建议（首帧已给）

**共识 1（最强、最一致）：首帧已提供主体/场景/风格，prompt 应优先描述运动/变化与运镜，避免重复复述静态画面；必要时补充环境动态或风格过渡。** 这是本次调研跨厂商一致性最高的一条，且直接命中 MatrixSpooll 的 i2v 路径。
...
> **对 MatrixSpooll 的启示（仅陈述共性事实）**："video_prompt 描述该镜头时长内的动作/运镜"与官方"短镜头聚焦单一、可完成动作 + 显式运镜"的主流口径一致；"低缓连续"仅是部分供应商（如火山 Seedance）的具体建议，"环境音"仅在供应商明确支持时适用，均不宜归为全体主流口径。"9 秒镜头动作仅 28 字、缺环境动态层"的问题，可对照官方"运动+运镜+（环境/氛围动态）"分层来衡量——官方既反对复述静态画面，也反对堆多个同时剧烈动作。
...
- Google 图像（Gemini/Nano Banana，正是 MatrixSpooll 首帧用途）：逐字建议 `Describe what you want, not what you don't`，示例把 "no cars" 改写为 "an empty, deserted street with no signs of traffic"（https://cloud.google.com/gemini-enterprise-agent-platform/models/capabilities/gemini-image-generation-best-practices ）。
...
> **对 MatrixSpooll 三个冻结决策的直接关联（仅提供事实依据，不给改稿）**：
...
> - 是否保留否定句——取决于目标模型：MatrixSpooll 若首帧走 Gemini/Seedream 类，官方倾向正向改写；若走支持 negative_prompt 的模型（可灵/qwen-image/wan2.7 视频），可用独立通道但仍以"描述式"为佳。
...
> **对 MatrixSpooll 的启示**：官方层面没有"中文 vs 英文哪个更好"的可引用定论。国产模型（火山/阿里/可灵/MiniMax/Vidu）官方文档中文并重、示例中文充分；海外模型（Google/OpenAI/xAI）示例以英文为主但未强制。MatrixSpooll 的 prompt 语言选择缺少官方硬依据，属可自行决策项。
...
1. **否定式处理三分天下（最大分歧）**：
   ...
   - 完全无口径：xAI。
   → **同一厂商内也可能分裂**：Google（图像无/视频有）、阿里（模型间有/无）。MatrixSpooll 的否定式策略无法"一刀切"，需按目标模型区分。
...
5. **Vidu"Q3 官方提示词指南"来源存疑（例外，需核实）**：中文侧署名"Vidu API 开放平台"的《结构公式+速查模板（50–150 字、音效句尾、i2v 分阶段结构）》仅在 CSDN/gitcode 镜像可见，**未能定位 platform.vidu.cn/.com 或 vidu.com/blog 官方原始 URL**。本报告未将其纳入结论；若 MatrixSpooll 要引用其"50–150 字"等具体口径，建议先向 Vidu 官方核实原页。

6. **语言优劣普遍空白（例外是没有例外）**：8 家中无一家声明"中文/英文哪个更好"。唯一有语言机制明文的 Google Imagen 属已弃用旧线。这与国产模型"中文优化"的坊间印象不符——**官方文档层面并无此声明**，属 MatrixSpooll 的自主决策项。
...
- 本次不含 MatrixSpooll 指导语改稿方案——仅交调研事实与共性结论，改稿由主仓库会话在对齐后另行制定。
