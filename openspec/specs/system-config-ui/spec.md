...
系统配置页（`SystemConfigPage`，路由 `/app/settings`，分区由 `?section=` query 控制）SHALL 以左侧栏导航在以下分区间切换：**供应商**（providers）、**MatrixSpooll 智能体配置**（agent）、**AI 生图/生视频配置**（media）、**用量统计**（usage）、**API Keys**（api-keys）、**关于**（about）。

各分区承载内容：
- **供应商**：预置 / 自定义供应商配置管理
- **MatrixSpooll 智能体配置**：Anthropic API Key、Base URL、各模型选择字段，以及速率限制 / 并发 Worker 等高级运行调优（advanced settings 折叠在本分区内，无独立分区）
