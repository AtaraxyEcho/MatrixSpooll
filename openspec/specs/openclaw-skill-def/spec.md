...
- **THEN** 系统返回渲染后的 Skill 定义文件，其中所有 `{{BASE_URL}}` 已被替换为实际的服务地址（如 `https://my-matrixspooll.example.com`）
...
### Requirement: 认证说明
skill.md SHALL 说明认证方式：用户从 MatrixSpooll 设置页面获取 API Key（`msp-` 前缀），通过 `Authorization: Bearer <API_KEY>` 传递。
