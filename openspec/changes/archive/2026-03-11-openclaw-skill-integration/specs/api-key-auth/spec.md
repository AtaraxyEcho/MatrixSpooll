...
系统 SHALL 提供 API Key 创建接口，生成格式为 `msp-` + 32 位随机字符串的密钥，返回完整密钥（仅在创建时可见），数据库仅存储 SHA-256 哈希值。
...
系统 SHALL 在 `_verify_and_get_payload` 中根据 token 前缀判定认证模式：以 `msp-` 开头走 API Key 验证路径，否则走 JWT 验证路径。

#### Scenario: API Key 认证成功
- **WHEN** 请求携带 `Authorization: Bearer msp-xxxxx` 且该 key 在数据库中存在且未过期
...
- **WHEN** 请求携带 `msp-` 前缀 token 但哈希未匹配到数据库记录
- **THEN** 系统返回 401

#### Scenario: JWT 认证不受影响
- **WHEN** 请求携带不以 `msp-` 开头的 Bearer token
