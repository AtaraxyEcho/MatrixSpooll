...
- [x] 2.1 修改 `server/auth.py` 的 `_verify_and_get_payload`，以 `msp-` 前缀判定走 API Key 或 JWT 路径
...
- [x] 3.2 实现 API Key 生成逻辑（`msp-` + 32 位随机字符串，哈希存储，创建时返回完整 key）
...
- [x] 5.1 创建 `public/skill.md.template`，参考 Zopia 格式编写 MatrixSpooll Skill 定义，API URL 使用 `{{BASE_URL}}` 占位符
