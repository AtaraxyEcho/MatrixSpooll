...
MatrixSpooll 当前使用 OAuth2 Bearer JWT 认证，所有 API 端点通过 `get_current_user` 依赖验证。前端先通过 `/auth/token` 获取 JWT，后续请求携带 `Authorization: Bearer <jwt>`。

OpenClaw 等外部平台需要长期有效的 API Key 来调用 MatrixSpooll API，而非短期 JWT。此外，现有助手对话基于 SSE 流式，外部 Agent 需要同步请求-响应接口。
...
**决定**: 使用 `msp-` 前缀 + 32 位随机字符串，数据库只存 SHA-256 哈希。

格式：`msp-xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx`（36 字符）

**理由**: 前缀便于用户识别来源，同时作为认证分流的判断依据；哈希存储确保数据库泄露不会暴露原始 key。参考 Zopia 的 `zopia-xxxxxxxxxxxx` 模式。

### 2. 认证层改造方式

**决定**: 修改 `server/auth.py` 的 `_verify_and_get_payload`，通过 `msp-` 前缀直接判定认证模式。

流程：
1. 提取 Bearer token
2. 检查 token 是否以 `msp-` 开头
...
- `name`: 用户自定义名称
- `key_hash`: SHA-256 哈希
- `key_prefix`: 前 8 位（`msp-xxxx`）用于列表展示
- `created_at`: 创建时间
