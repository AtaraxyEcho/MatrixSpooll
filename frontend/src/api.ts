...
export interface CreateProjectPayload {
  title: string;
  name?: string;
  content_mode?: "narration" | "drama" | "ad" | "free";
  /** 源文件性质：novel（默认）/ screenplay。仅 drama 暴露，创建即定、不可变。 */
  source_kind?: "novel" | "screenplay";
  aspect_ratio?: string;
  /** 固定工作流创建时必填二选一；自由创作显式为 null。 */
  generation_mode: GenerationRoute | null;
...
class API {
  ...
  static async cancelFreeCreation(
    ...
    return this.request(
      `/projects/${encodeURIComponent(projectName)}/creations/${encodeURIComponent(creationId)}/cancel`,
      { method: "POST" },
    );
  }
