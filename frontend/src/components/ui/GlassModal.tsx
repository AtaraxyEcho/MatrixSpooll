...
export function GlassModal(props: GlassModalProps) {
  ...
  return (
    <ModalShell
      ...
      className={`msp-glass-panel overflow-hidden rounded-2xl ${widthClassName} ${panelClassName}`.trim()}
      style={panelStyle}
      {...shellA11y}
    >
      <span aria-hidden="true" className="msp-glass-hairline" data-tone={hairlineTone} />
      {children}
    </ModalShell>
  );
}
