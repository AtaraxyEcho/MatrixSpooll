...
export interface WizardStep1Value {
  title: string;
  contentMode: "narration" | "drama" | "ad" | "free";
...
export function WizardStep1Basics({
  ...
  const handleNext = () => {
    ...
    // 固定工作流路线必选；自由创作没有项目级 generation_mode。
    if (value.contentMode !== "free" && !value.generationRoute) return;
    ...
    <div className="space-y-5">
      ...
      <div>
        ...
        <p className="mt-2 text-[11.5px] leading-[1.55] text-text-3">
          {value.contentMode === "free"
            ? t("dashboard:content_mode_free_desc")
      ...
      <div className="mt-7 flex items-center justify-between border-t border-hairline-soft pt-5">
        ...
        <button
          type="button"
          onClick={handleNext}
          disabled={!value.title.trim() || (value.contentMode !== "free" && !value.generationRoute)}
