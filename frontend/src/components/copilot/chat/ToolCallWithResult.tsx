...
export function ToolCallWithResult({ block }: ToolCallWithResultProps) {
  ...
  // MatrixSpooll in-process MCP tool 显示名：从 mcp__matrixspooll__<id> 中提取 id，
  // 查 dashboard:tool_name_<id>（单一真相源 = backend MATRIXSPOOLL_MCP_TOOL_IDS）。
  // AskUserQuestion 显示为本地化「提问」标签；其余工具（Bash / TodoWrite /
  // Skill / ...）保留原名。
  const mcpMatch = /^mcp__matrixspooll__([a-z0-9_]+)$/.exec(toolName);
