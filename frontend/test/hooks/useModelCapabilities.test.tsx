...
describe("useGenerationCapabilities", () => {
  it("switches duration and resolution options with the exact selected model", async () => {
    const spy = vi.spyOn(API, "getFreeCreationCapabilities").mockImplementation(async ({ model }) => ({
      output_type: "video",
      model: model ?? "custom-99/seedance-2.0",
      ratios: ["16:9"],
      resolutions: model?.endsWith("seedance-2.5")
        ? ["480p", "720p", "1080p"]
        : ["720p", "1080p", "2k"],
      durations: model?.endsWith("seedance-2.5")
    ...
    const { result, rerender } = renderHook(
      ({ model }: { model: string }) => useGenerationCapabilities({ outputType: "video", model }),
      { initialProps: { model: "custom-99/seedance-2.0-ultra" } },
    );

    await waitFor(() => expect(result.current.capabilities?.resolutions).toEqual(["720p", "1080p", "2k"]));
    expect(result.current.capabilities?.durations.at(-1)).toBe(15);

    rerender({ model: "custom-99/seedance-2.5" });
    expect(result.current.capabilities).toBeNull();
    expect(result.current.loading).toBe(true);
    await waitFor(() => expect(result.current.capabilities?.durations.at(-1)).toBe(30));
    expect(result.current.capabilities?.resolutions).toEqual(["480p", "720p", "1080p"]);
    expect(spy).toHaveBeenCalledTimes(2);
  });

  it("refetches when provider capabilities change", async () => {
    const spy = vi.spyOn(API, "getFreeCreationCapabilities").mockResolvedValue({
      output_type: "video",
      model: "custom-98/seedance-2.5",
      ratios: ["16:9"],
      resolutions: ["1080p"],
      durations: [4, 30],
      max_reference_images: 30,
      max_reference_videos: 0,
      max_reference_media_count: 30,
    });
  ...
  it("uses the model descriptor endpoint when generation-mode validation is disabled", async () => {
    const strictSpy = vi.spyOn(API, "getFreeCreationCapabilities");
    const modelSpy = vi.spyOn(API, "getModelCapabilities").mockResolvedValue({
      output_type: "video",
      model: "custom-97/i2v-only",
      ratios: ["16:9"],
      resolutions: ["720p"],
      durations: [4, 8],
    ...
    await waitFor(() => expect(result.current.capabilities?.text_to_video).toBe(false));
