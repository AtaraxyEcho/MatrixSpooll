...
import { anchorSelector, startTour, type TourLabels, type TourStep } from "@/onboarding/tour";
...
describe("startTour", () => {
  ...
  it("renders one filmstrip cell per step, filled up to the current step", () => {
    ...
    const cells = () => Array.from(popover().querySelectorAll<HTMLElement>(".msp-tour-filmstrip span"));
  ...
  it("states the step position in text for screen readers", () => {
    ...
    expect(popover().querySelector(".msp-tour-sr-only")?.textContent).toBe("第 1 步，共 2 步");
  ...
  it("offers skip on every step but the last", () => {
    ...
    expect(popover().querySelector(".msp-tour-skip-btn")?.textContent).toBe("跳过");

    click(".driver-popover-next-btn");
    expect(popover().querySelector(".msp-tour-skip-btn")).toBeNull();
