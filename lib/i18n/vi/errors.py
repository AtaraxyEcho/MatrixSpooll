MESSAGES = {
    ...
    "admin_username_length": "Tên người dùng phải có từ {min} đến {max} ký tự",
    "admin_username_start": "Tên người dùng phải bắt đầu bằng chữ cái",
    "admin_username_characters": "Tên người dùng chỉ được chứa chữ cái tiếng Anh, chữ số, ., _, hoặc -",
    "admin_username_separators": "Tên người dùng không được kết thúc bằng dấu phân cách hoặc chứa các dấu phân cách liên tiếp",
    "admin_user_exists": "Tên người dùng đã tồn tại",
    "admin_nickname_length": "Tên hiển thị phải có từ {min} đến {max} ký tự",
    "admin_nickname_characters": "Tên hiển thị chỉ chứa chữ cái Trung/Anh, chữ số, gạch dưới, gạch nối, dấu chấm giữa (không có dấu cách)",
    "admin_nickname_banned": "Tên hiển thị chứa từ bị cấm",
    ...
    "admin_task_not_found": "Không tìm thấy tác vụ",
    "admin_task_not_active": "Tác vụ không còn ở trạng thái có thể hủy",
    "admin_task_retry_terminal_only": "Chỉ có thể thử lại tác vụ đã thất bại hoặc bị hủy",
    "avatar_too_large": "Tệp ảnh đại diện quá lớn (tối đa 2MB)",
    "session_invalid": "Phiên đăng nhập đã hết hạn, vui lòng đăng nhập lại",
    "token_legacy_rejected": "Định dạng thông tin đăng nhập không còn được chấp nhận, vui lòng đăng nhập lại",
    "download_access_denied": "Thông tin tải xuống không còn hợp lệ hoặc bạn không có quyền truy cập dự án này",
    "pricing_unconfigured": "Mô hình này chưa cấu hình giá hợp lệ; chi phí chưa được ghi nhận",
    "provider_base_url_forbidden": "Địa chỉ nhà cung cấp không được phép: {reason}",
    ...
    "login_credentials_required": "Nhập tên đăng nhập và mật khẩu",
    ...
    "free_creation_subtitle_render_invalid": "Track phụ đề hoặc video nguồn không hợp lệ",
    "free_creation_subtitle_render_unavailable": "Không thể tạo video có phụ đề vì FFmpeg chưa được cấu hình hoặc video không thể xử lý",
    ...
    "free_creation_storyboard_image_required": "Hãy tạo ảnh storyboard cho mọi cảnh đã chọn trước",
    "free_creation_audio_unavailable": "Dự án chưa cấu hình dịch vụ tạo giọng nói",
    "free_creation_voice_unsupported": "Backend âm thanh hiện tại không hỗ trợ giọng đã chọn",
    "free_creation_subtitle_video_required": "Chỉ có thể gắn phụ đề vào video đã hoàn tất",
    "free_creation_subtitle_not_found": "Không tìm thấy track phụ đề '{id}'",
    "free_creation_subtitle_conflict": "Track phụ đề đã thay đổi ở nơi khác, hãy tải lại trước khi lưu",
    "free_creation_merge_input_not_found": "Một video đã chọn không tồn tại hoặc không còn khả dụng",
    "free_creation_merge_invalid": "Hãy chọn ít nhất hai video đã hoàn tất để ghép",
    "free_creation_merge_unavailable": "Không thể ghép video vì FFmpeg chưa được cấu hình hoặc các đoạn không thể kết hợp",
    "free_creation_merge_tools_missing": "Ghép video cần FFmpeg và FFprobe. Hãy cài đặt cả hai công cụ rồi khởi động lại dịch vụ backend.",
    "free_creation_merge_input_unreadable": "Một video đã chọn bị hỏng hoặc thông tin phương tiện của video không thể đọc được.",
    "free_creation_merge_limits_exceeded": "Các video đã chọn vượt quá giới hạn kích thước hoặc thời lượng ghép. Hãy chọn ít đoạn hơn rồi thử lại.",
    "invalid_image_format": "Định dạng ảnh không được hỗ trợ",
    ...
    "project_access_denied": "Bạn không có quyền truy cập dự án '{name}'",
    "project_member_user_not_found": "Người dùng không tồn tại hoặc đã bị vô hiệu hóa",
    "project_member_exists": "Người dùng này đã là thành viên dự án",
    "project_member_not_found": "Thành viên dự án không tồn tại",
    "project_owner_change_requires_transfer": "Thay đổi chủ sở hữu dự án cần thao tác chuyển quyền riêng",
    "project_owner_cannot_be_removed": "Không thể xóa chủ sở hữu dự án",
    ...
    "video_first_frame_content_rejected": "Khung hình đầu không vượt qua kiểm duyệt của nhà cung cấp: ảnh có thể chứa nội dung nhạy cảm, chân dung người thật hoặc thông tin riêng tư. Hãy thay khung hình đầu; với tư liệu người thật đã được cấp quyền, hãy dùng quy trình xác minh danh tính hoặc tài sản được cấp phép mà nhà cung cấp hỗ trợ",
    "video_last_frame_content_rejected": "Khung hình cuối không vượt qua kiểm duyệt của nhà cung cấp: ảnh có thể chứa nội dung nhạy cảm, chân dung người thật hoặc thông tin riêng tư. Hãy thay khung hình cuối; với tư liệu người thật đã được cấp quyền, hãy dùng quy trình xác minh danh tính hoặc tài sản được cấp phép mà nhà cung cấp hỗ trợ",
    "video_reference_image_content_rejected": "Ảnh tham chiếu số {number} không vượt qua kiểm duyệt của nhà cung cấp: ảnh có thể chứa nội dung nhạy cảm, chân dung người thật hoặc thông tin riêng tư. Hãy thay ảnh tham chiếu đó; với tư liệu người thật đã được cấp quyền, hãy dùng quy trình xác minh danh tính hoặc tài sản được cấp phép mà nhà cung cấp hỗ trợ",
    "video_input_image_content_rejected": "Một ảnh đầu vào không vượt qua kiểm duyệt của nhà cung cấp: ảnh có thể chứa nội dung nhạy cảm, chân dung người thật hoặc thông tin riêng tư. Hãy thay ảnh đầu vào; với tư liệu người thật đã được cấp quyền, hãy dùng quy trình xác minh danh tính hoặc tài sản được cấp phép mà nhà cung cấp hỗ trợ",
    "video_input_text_content_rejected": "Câu lệnh không vượt qua kiểm duyệt nội dung của nhà cung cấp. Hãy sửa các mô tả có thể liên quan đến nội dung nhạy cảm rồi thử lại",
    "video_output_content_rejected": "Kết quả được tạo không vượt qua kiểm duyệt nội dung của nhà cung cấp. Hãy điều chỉnh câu lệnh hoặc tư liệu tham chiếu rồi thử lại",
    ...
    "free_creation_reference_role_unsupported": "Mô hình hoặc loại đầu ra hiện tại không hỗ trợ vai trò {role}",
    "free_creation_t2v_unsupported": "{provider}/{model} không hỗ trợ tạo video từ văn bản; hãy thêm đầu vào hình ảnh phù hợp hoặc đổi mô hình",
    "free_creation_first_frame_unsupported": "Mô hình {model} không hỗ trợ tạo từ khung hình đầu",
    "free_creation_last_frame_unsupported": "Mô hình {model} không hỗ trợ tạo từ khung hình đầu và cuối",
    "free_creation_reference_images_unsupported": "Mô hình {model} không hỗ trợ ảnh tham chiếu",
    "free_creation_reference_videos_unsupported": "Mô hình {model} không hỗ trợ video tham chiếu",
    "free_creation_reference_audio_unsupported": "Mô hình {model} không hỗ trợ âm thanh tham chiếu",
    ...
    "free_creation_cover_not_found": "Không tìm thấy ảnh bìa của tạo tác tự do '{id}'",
    ...
    "free_creation_delete_not_ready": "Không thể xóa tạo tác tự do đang chạy; hãy hủy tác vụ trước",
    ...
    "video_reference_videos_duration_exceeded": "Mô hình {model} chỉ cho phép tổng thời lượng video tham chiếu tối đa {limit} giây nhưng nhận {total} giây; hãy giảm hoặc cắt clip",
    "video_reference_video_duration_unsupported": "Video tham chiếu {index} của mô hình {model} phải dài từ {min} đến {max} giây nhưng hiện dài {duration} giây",
    ...
    "video_resolution_required": "Mô hình {model} yêu cầu chọn độ phân giải",
