MESSAGES = {
    ...
    "admin_username_length": "用户名长度必须为 {min}–{max} 位",
    "admin_username_start": "用户名必须以英文字母开头",
    "admin_username_characters": "用户名仅支持英文字母、数字及 . _ -",
    "admin_username_separators": "用户名不能以符号结尾或连续使用符号",
    "admin_user_exists": "用户名已存在",
    "admin_nickname_length": "昵称长度需为 {min}–{max} 个字符",
    "admin_nickname_characters": "昵称仅支持中英文、数字、下划线、连字符、中点（不含空格）",
    "admin_nickname_banned": "昵称包含受限词汇，请更换",
    ...
    "admin_session_not_found": "登录会话不存在",
    "admin_task_not_found": "任务不存在",
    "admin_task_not_active": "任务已经不是可取消状态",
    "admin_task_retry_terminal_only": "只有失败或已取消的任务可以重试",
    "avatar_too_large": "头像文件过大，最大支持 2MB",
    "session_invalid": "登录会话已失效，请重新登录",
    "token_legacy_rejected": "登录凭证格式已升级，请重新登录",
    "download_access_denied": "下载凭证已失效或无权访问该项目",
    "pricing_unconfigured": "该模型未配置有效定价，费用未入账",
    "provider_base_url_forbidden": "供应商地址不被允许：{reason}",
    "api_key_invalid": "API Key 无效、已过期、已吊销或不可用",
    ...
    "login_credentials_required": "请输入用户名和密码",
    ...
    "free_creation_subtitle_render_invalid": "字幕轨道或其来源视频无效",
    "free_creation_subtitle_render_unavailable": "字幕生成不可用，请先配置 FFmpeg 或检查视频是否能够处理",
    "free_creation_storyboard_source_invalid": "分镜来源为空或不是支持的文本资源",
    ...
    "free_creation_merge_input_not_found": "选择的视频片段中有文件不存在或已不可用",
    "free_creation_merge_invalid": "请选择至少两个已完成的视频片段进行合并",
    "free_creation_merge_unavailable": "视频合并不可用，请先配置 FFmpeg 或检查片段是否能够合并",
    "free_creation_merge_tools_missing": "视频合并需要 FFmpeg 和 FFprobe，请完成安装并重启后端服务",
    "free_creation_merge_input_unreadable": "所选视频已损坏，或其媒体信息无法被读取",
    "free_creation_merge_limits_exceeded": "所选视频总大小或总时长超过合并限制，请减少片段后重试",
    ...
    "project_access_denied": "你没有项目 '{name}' 的访问权限",
    "project_identifier_ambiguous": "项目名称“{name}”对应多个项目，请使用项目 ID",
    "project_delete_active_tasks": "当前项目仍有 {count} 个生成任务正在执行，请先取消任务并等待任务结束后再删除",
    "project_member_user_not_found": "用户不存在或已停用",
    "project_member_exists": "该用户已经是项目成员",
    "project_member_not_found": "项目成员不存在",
    "project_owner_change_requires_transfer": "项目所有者变更需要单独的转移操作",
    "project_owner_cannot_be_removed": "项目所有者不能被移除",
    ...
    "video_first_frame_content_rejected": "首帧未通过供应商审核：图片可能包含敏感内容、真人肖像或隐私信息。请更换首帧；如使用已获授权的真人素材，请改用供应商支持的真人验证或授权资产流程",
    "video_last_frame_content_rejected": "尾帧未通过供应商审核：图片可能包含敏感内容、真人肖像或隐私信息。请更换尾帧；如使用已获授权的真人素材，请改用供应商支持的真人验证或授权资产流程",
    "video_reference_image_content_rejected": "第 {number} 张参考图未通过供应商审核：图片可能包含敏感内容、真人肖像或隐私信息。请更换该参考图；如使用已获授权的真人素材，请改用供应商支持的真人验证或授权资产流程",
    "video_input_image_content_rejected": "输入图片未通过供应商审核：图片可能包含敏感内容、真人肖像或隐私信息。请更换输入图片；如使用已获授权的真人素材，请改用供应商支持的真人验证或授权资产流程",
    "video_input_text_content_rejected": "提示词未通过供应商内容审核。请修改可能涉及敏感内容的描述后重试",
    "video_output_content_rejected": "生成结果未通过供应商内容审核。请调整提示词或参考素材后重试",
    ...
    "free_creation_reference_role_unsupported": "当前模型或输出类型不支持 {role} 素材用途",
    "free_creation_t2v_unsupported": "{provider}/{model} 不支持文生视频；请添加兼容的视觉素材或切换模型",
    "free_creation_first_frame_unsupported": "模型 {model} 不支持首帧生成",
    "free_creation_last_frame_unsupported": "模型 {model} 不支持首尾帧生成",
    "free_creation_reference_images_unsupported": "模型 {model} 不支持参考图片",
    "free_creation_reference_videos_unsupported": "模型 {model} 不支持参考视频",
    "free_creation_reference_audio_unsupported": "模型 {model} 不支持参考音频",
    "free_creation_input_combination_unsupported": "模型 {model} 不支持当前素材用途组合",
    ...
    "video_reference_videos_duration_exceeded": "模型 {model} 的参考视频总时长最多为 {limit} 秒，当前为 {total} 秒；请减少或裁剪参考视频",
    "video_reference_video_duration_unsupported": "模型 {model} 的第 {index} 段参考视频必须为 {min} 至 {max} 秒，当前为 {duration} 秒",
    ...
    "video_resolution_required": "模型 {model} 要求选择分辨率",
