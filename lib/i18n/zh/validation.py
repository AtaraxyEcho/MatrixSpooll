...
MESSAGES = {
    ...
    "val_field_must_be_empty": "{field} 必须为空数组",
    ...
    "arch_upload_too_large": "项目归档超过上传上限 {max_mb} MB",
    "arch_zip_too_many_entries": "ZIP 条目数量超过上限 {max_count}",
    "arch_zip_entry_too_large": "ZIP 条目超过单文件上限 {max_mb} MB: {name}",
    "arch_zip_expanded_too_large": "ZIP 解压后的总大小超过上限 {max_mb} MB",
    "arch_zip_compression_ratio_too_high": "ZIP 条目压缩比异常（上限 {max_ratio}:1）: {name}",
    "arch_multiple_manifests": "ZIP 中包含多个 matrixspooll-export.json，无法确定项目根目录",
    ...
    "val_aspect_ratio_invalid": "画面比例必须是正整数宽高比，当前为 {value}",
}
