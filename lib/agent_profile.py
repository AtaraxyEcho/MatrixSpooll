...
Default: ``<PROJECT_ROOT>/agent_runtime_profile``. Set ``MATRIXSPOOLL_PROFILE_DIR``
...
def agent_profile_dir() -> Path:
    override = os.getenv("MATRIXSPOOLL_PROFILE_DIR", "").strip() or os.getenv("ARCREEL_PROFILE_DIR", "").strip()
    if override:
