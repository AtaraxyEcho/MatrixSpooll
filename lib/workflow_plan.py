...
def build_workflow_plan(
    ...
    *,
    ...
    if not status.project.workflow_applicable:
        return WorkflowPlan(
            status=status,
            narration_delivery=WorkflowNarrationDelivery(selected=None),
            steps=[],
    ...
    if status.project.generation_mode is None:
        raise ValueError("fixed workflow status is missing generation_mode")
