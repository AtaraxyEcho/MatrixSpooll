...
_VALID_RATIO_RE = re.compile(r"^\s*[1-9]\d*\s*[:：]\s*[1-9]\d*\s*$")


def is_valid_aspect_ratio(value: object) -> bool:
    """Return whether ``value`` is a positive integer ``width:height`` ratio."""
    return isinstance(value, str) and _VALID_RATIO_RE.fullmatch(value) is not None
