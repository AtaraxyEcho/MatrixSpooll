...
ProjectPhase = Literal["preparation", "script", "production", "completed", "creative"]
...
class WorkflowStateService:
    ...
    def get_status(self, project_name: str, episode: int | None = None) -> WorkflowStatus:
        ...
        if project.get("content_mode") == "free":
            if episode is not None:
                raise WorkflowRequestError("free creation projects do not have episodes")
            return self._free_status(project)
    ...
    @staticmethod
    def _free_status(project: dict[str, Any]) -> WorkflowStatus:
        """Return a non-workflow status for a free creation project."""
        return WorkflowStatus(
            ...
            blockers=[],
            gates={},
            artifacts={"free_creations": {"state": "available"}},
            next_action=_action(WorkflowActionType.NONE, "free creation projects use the creations API"),
        )
    ...
    def get_project_summary(
        ...
        *,
        ...
        project_path = self.pm.get_project_path(project_name)
        if project.get("content_mode") == "free":
            assets = self._asset_counts(project_path, project, None)
            return ProjectSummary(
                ...
                repair_reason=None,
                assets=assets,
                episodes_summary=_episodes_summary([]),
                episodes=[],
                workflow_applicable=False,
            )
