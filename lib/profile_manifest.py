...
manifest 落在 ``{project_dir}/.matrixspooll_profile_manifest.json``（项目根，跨 .claude
...
MANIFEST_FILENAME = ".matrixspooll_profile_manifest.json"
LOCK_FILENAME = ".profile_sync.lock"
MANIFEST_SCHEMA_VERSION = 1
EXPECTED_PROFILE_ID = "matrixspooll/builtin"
...
ContentMode = Literal["narration", "drama", "ad", "free"]
VALID_CONTENT_MODES: frozenset[str] = frozenset({"narration", "drama", "ad", "free"})
...
def _project_lock(project_dir: Path):
    ...
    或开发者模式，攻击模型本就低；预检与 open 之间的 TOCTOU 窗口与 MatrixSpooll
...
def _safe_copy(source: Path, dest: Path) -> None:
    ...
    symlink 时仍跟。MatrixSpooll 项目目录由 server 自创、普通用户无 shell + portalocker
