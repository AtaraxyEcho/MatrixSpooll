...
def _file_logging_disabled() -> bool:
    value = os.environ.get("MATRIXSPOOLL_LOG_FILE_DISABLED", os.environ.get("ARCREEL_LOG_FILE_DISABLED", ""))
    return value.strip().lower() in _DISABLED_TRUTHY


def resolve_log_dir() -> Path:
    """日志目录解析：MATRIXSPOOLL_LOG_DIR > PROJECT_ROOT/logs。
    ...
    """
    raw = os.environ.get("MATRIXSPOOLL_LOG_DIR", os.environ.get("ARCREEL_LOG_DIR", "")).strip()
    if raw:
        path = Path(raw)
...
def migrate_legacy_log_dir() -> None:
    """将旧默认位置的日志迁到新位置；只在 MATRIXSPOOLL_LOG_DIR 未显式覆盖时进行。

    策略：
    - 用户显式设了 MATRIXSPOOLL_LOG_DIR → 不动（用户已自主决定路径）
    - 新旧路径解析到同一处 → no-op（例如 MATRIXSPOOLL_DATA_DIR == PROJECT_ROOT）
    - 旧目录不存在 → no-op
    ...
    """
    if os.environ.get("MATRIXSPOOLL_LOG_DIR", os.environ.get("ARCREEL_LOG_DIR", "")).strip():
        return
