...
- **README 保持稳定**：README 只需让第一次访问仓库的人回答「MatrixSpooll 是什么、适不适合我、和直接调模型 API 有什么区别、怎样最快跑起来」。具体模型名称、单价和接口参数放到站点对应页面，避免供应商每次更新都要重写首页。
- **供应商信息以运行时能力为准**：文档描述覆盖哪些媒体类型、MatrixSpooll 如何统一配置、不同能力如何选择、具体信息在哪里确认；设置页中实际可选的模型与供应商官方文档是最终依据。
