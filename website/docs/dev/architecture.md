...
本文档描述 MatrixSpooll 的稳定架构边界、主要数据流和扩展点。它不替代代码级 API 文档，也不记录临时实现计划。

## 1. 架构目标 {#goals}

MatrixSpooll 的核心目标不是绑定某个模型，而是提供一条：
...
MatrixSpooll 的项目不仅是一条数据库记录，还包括文件系统中的媒体资产。
...
“可继续编辑”是 MatrixSpooll 与只输出单个视频文件的生成工具之间的重要差异。
...
MatrixSpooll 在支持的环境中使用 `bwrap` 等机制限制这些能力。Docker Compose 为沙箱配置了额外权限，因此生产部署需要在功能和宿主机隔离之间做清晰取舍。
