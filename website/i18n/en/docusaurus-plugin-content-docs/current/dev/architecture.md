...
This document describes MatrixSpooll's stable architectural boundaries, primary data flows, and extension points. It does not replace code-level API documentation or record temporary implementation plans.

## 1. Architecture Goals {#goals}

MatrixSpooll's core goal is not to tie the product to any particular model, but to provide an AI video production pipeline that is:
...
A MatrixSpooll project is more than a database record; it also includes media assets in the file system.
...
- MatrixSpooll's records do not replace official provider invoices.
...
The ability to continue editing is an important difference between MatrixSpooll and generation tools that output only a single video file.
...
MatrixSpooll uses mechanisms such as `bwrap` to restrict these capabilities in supported environments. Docker Compose configures additional permissions for the sandbox, so production deployments must make a clear tradeoff between functionality and host isolation.
...
## 16. Extending MatrixSpooll with a New Provider {#extend-provider}
...
## 17. Extending MatrixSpooll with a New Workflow Stage {#extend-workflow-stage}
