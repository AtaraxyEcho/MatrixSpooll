...
Export video clips generated by MatrixSpooll as a Jianying draft for each episode, then open the draft directly in Jianying Desktop for further editing—adjust the pacing, add subtitles, transitions, voice-over, and more.

MatrixSpooll currently exports the draft format for the mainland-China edition of Jianying Desktop. CapCut is Jianying's international counterpart, but it is a separate product; MatrixSpooll has not verified draft compatibility with CapCut.

## Prerequisites {#prerequisites}

- Video clips have been generated for at least one episode in MatrixSpooll
...
### 2. Start the export in MatrixSpooll {#start-export}
...
Jianying export, the workspace presentation preview, and editable bundle downloads all read the same presentation selection. Video and optional TTS tracks use the selected versions, and track lengths come from the media files' actual durations. Original audio and TTS remain at their original speed and unity gain; MatrixSpooll does not truncate, time-stretch, mix, or re-encode them. Mechanical subtitle timing is allocated from the actual media boundary and text length, and remains adjustable or replaceable in Jianying.

A manually uploaded video without generation provenance is exported unchanged and is explicitly marked as having unavailable provenance. MatrixSpooll does not infer its currency or generate TTS or subtitles for it.
