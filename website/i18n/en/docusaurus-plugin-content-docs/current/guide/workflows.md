...
MatrixSpooll supports fixed workflows and project-scoped free creation. This page helps you choose the right path before starting a project and define the review criteria for each stage.
...
2. **Video-Making Workflow**: required only for fixed workflow projects; it determines whether production is organized around storyboard images or asset reference images.

Fixed `drama`, `narration`, and `ad` projects must choose `storyboard` or `reference_video` when they are created, and the choice cannot be changed later. `free` is a content mode, not a third generation route: free projects do not choose a route and store `generation_mode: null`.
...
Free creation projects do not need source files, scripts, episodes, or fixed workflow steps. Enter a prompt in the project canvas and choose image, video, or image edit. Video requests go directly to the video model; reference images or audio are included only when supplied, without forcing an intermediate storyboard image.
...
### 3.4 Free Creation Mode {#free-mode}

Best for:

- Testing a visual or motion idea directly;
- Creating a single image or short video without scripts, episodes, or storyboards;
- Editing an existing image or using it as a direct video reference;
- Keeping prompts, reference paths, and versions inside the same project.

The free canvas supports image, video, and image-edit outputs. The default direct path sends the original prompt to the image or video model without an extra text-model call. A chat entry may use a text model for context, routing, and future prompt enhancement, but that is an orchestration choice rather than a mandatory media step.

The first ratio presets are `9:16`, `16:9`, `1:1`, `4:3`, `3:4`, and `21:9`, with positive-integer `width:height` validation. The active provider/model remains the source of truth for supported ratios; unsupported values should fail before queueing instead of silently falling back.
...
| First MatrixSpooll trial | Any | Storyboard image-to-video |
...
MatrixSpooll determines an artifact's state from the direct inputs used to generate it:
...
- **Blocked**: A project reference or artifact record is damaged and must be repaired first. MatrixSpooll will not treat it as missing and automatically pay to regenerate it.
...
MatrixSpooll's advantage is not “skipping review,” but placing review where the cost of rework is lower.
